...
export const exportCandidates = async (req, res) => {
  try {
    ...
    const csvHeader = [
      'ID', 'First Name', 'Last Name', 'Email', 'Country Code', 'Mobile', 'Role', 'Status', 'Created At',
      'Application Type', 'Gender', 'Relationship Status', 'Address', 'Nationality', 'Birth Country', 'Place of Birth',
      'DOB', 'Passport Number', 'Issuing Authority', 'Issue Date', 'Expiry Date', 'Visa Type', 'BRP Number',
      'Visa Expiry Date', 'NI Number', 'Sponsored', 'English Proof'
    ];
    const csvRows = candidates.map(candidate => {
      ...
      return [
        ...
        app.issueDate ? new Date(app.issueDate).toISOString().split('T')[0] : '',
        app.expiryDate ? new Date(app.expiryDate).toISOString().split('T')[0] : '',
        app.visaType || '',
        app.brpNumber || '',
        app.visaEndDate ? new Date(app.visaEndDate).toISOString().split('T')[0] : '',
        app.niNumber || '',
        app.sponsored || '',
        app.englishProof || ''
      ];
  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      try {
        ...
        if (rowData['Application Type'] || rowData.gender || rowData.nationality || rowData.dob) {
          await CandidateApplication.create({
            userId: candidate.id,
            ...
            issueDate: rowData['Issue Date'] ? new Date(rowData['Issue Date']) : null,
            ...
            visaEndDate: rowData['Visa Expiry Date'] ? new Date(rowData['Visa Expiry Date']) : null,

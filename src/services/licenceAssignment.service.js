...
export function statusToAuditAction(status) {
  switch (String(status || "")) {
    case "Approved":
      return LICENCE_AUDIT_ACTIONS.APPROVE;
    case "Rejected":
      return LICENCE_AUDIT_ACTIONS.REJECT;
    case "Information Requested":
      return LICENCE_AUDIT_ACTIONS.REQUEST_INFO;
    case "Under Review":
      return LICENCE_AUDIT_ACTIONS.UNDER_REVIEW;
    default:
      return LICENCE_AUDIT_ACTIONS.REVIEW;
  }
}
...
export async function recordLicenceAudit({
  ...
  recordAuditLog({
    tenantDb,
    userId: actorId,
    action: `LICENCE_${String(action).toUpperCase()}`,
    ...
    logger.error({ err }, "Failed to mirror licence audit to global log")
  );
}

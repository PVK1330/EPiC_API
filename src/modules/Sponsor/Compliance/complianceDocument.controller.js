...
export const getComplianceDocuments = async (req, res) => {
    try {
        ...
        const documents = await req.tenantDb.Document.findAll({
            ...
            console.error('req.tenantDb.Document.findAll error:', err);
            return [];
        });

        // 2. Fetch from SponsorProfile and LicenceApplications (integrated docs)
        const [profile, licenceApps] = await Promise.all([
            req.tenantDb.SponsorProfile.findOne({ where: { userId } }).catch(() => null),
            req.tenantDb.LicenceApplication.findAll({ where: { userId } }).catch(() => [])
        ]);
...
export const deleteComplianceDocument = async (req, res) => {
    try {
        ...
        const document = await req.tenantDb.Document.findOne({ where: { id, userId } });

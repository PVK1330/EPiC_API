...
export const batchUpdateFieldVisibility = async (req, res) => {
  try {
    ...
    const updates = Object.entries(visibility).map(([field_key, is_visible]) => {
      return req.tenantDb.ApplicationFieldSetting.update(
        { is_visible },
        { where: { field_key } }
      );
    });
...
export const updateFieldVisibilityById = async (req, res) => {
  try {
    ...
    const setting = await req.tenantDb.ApplicationFieldSetting.findByPk(id);
    if (!setting) {
...
export const updateSingleFieldVisibility = async (req, res) => {
  try {
    ...
    const setting = await req.tenantDb.ApplicationFieldSetting.findOne({ where: { field_key } });
...
export const createCustomField = async (req, res) => {
  try {
    ...
    const existingField = await req.tenantDb.ApplicationCustomField.findOne({ where: { field_id } });
    if (existingField) {
      return res.status(400).json({
        status: "error",
    ...
    const customField = await req.tenantDb.ApplicationCustomField.create({
      field_id,
      label,
      field_type,
      placeholder,
...
export const updateCustomField = async (req, res) => {
  try {
    ...
    const customField = await req.tenantDb.ApplicationCustomField.findByPk(id);

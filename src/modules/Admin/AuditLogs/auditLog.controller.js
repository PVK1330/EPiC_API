...
export const getAuditLogs = async (req, res) => {
  try {
    ...
    // Fetch quick stats
    const totalCount = await req.tenantDb.AuditLog.count();
    const successCount = await req.tenantDb.AuditLog.count({ where: { status: 'Success' } });
    const failedCount = await req.tenantDb.AuditLog.count({ where: { status: 'Failed' } });
    
    const todayStart = new Date();
    todayStart.setHours(0,0,0,0);
    const todayCount = await req.tenantDb.AuditLog.count({
      where: {
        created_at: {
          [Op.gte]: todayStart
        }
...
export const exportAuditLogs = async (req, res) => {
  try {
    ...
    const logs = await req.tenantDb.AuditLog.findAll({
      where: whereClause,
      include: [{ model: req.tenantDb.User, as: 'user', attributes: ['first_name', 'last_name'] }],
      order: [["created_at", "DESC"]]
    });
